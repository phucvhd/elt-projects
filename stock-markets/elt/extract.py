...
class Extract:
    ...
    def extract_stock_prices_by_period(self, ticker: Ticker, period=None, interval=None) -> list[StockPrice]:
        try:
            print(f"Extracting stock prices by period from ticker: {ticker.ticker}")
            df = self.client.fetch_stock_prices_by_period(ticker, period, interval)
            return self.mapper.map_stock_price(df)
        except Exception as e:
            print("Error when extracting data", e)
            exit(1)

    def extract_stock_prices_by_date(self, ticker: Ticker, start="2023-01-01", end="2023-12-31") -> list[StockPrice]:
        try:
            print(f"Extracting stock prices by date from ticker: {ticker.ticker}, start: {start}, end: {end}")
            df = self.client.fetch_stock_prices_by_date(ticker, start, end)
            return self.mapper.map_stock_price(df)
        except Exception as e:
            print("Error when extracting data", e)
            exit(1)
